...
const swagger = 'swagger-ui-dist' // 'swagger-ui'
let modulesRootPath = require.resolve(swagger);
...
const init = module.exports = function (config) {
  return function () {
      ...
      res.format({
        ...
        'text/html': function () {
          ...
          if (typeof config.uiIndex === 'function') {
            ...
            if (req.query.url) {
              res.sendFile(path.join(modulesRootPath, 'node_modules/'+swagger+'/index.html'));
            } else {
    ...
    if (typeof config.uiIndex !== 'undefined') {
      const uiPath = path.dirname(require.resolve(swagger));
